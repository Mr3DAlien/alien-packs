...
execute anchored eyes run summon minecraft:snowball ^ ^ ^2 {Tags:["ap-rm_projectile"],Silent:1b,damage:0d,Item:{id:"minecraft:snowball",Count:1b,tag:{CustomModelData:1}},Passengers:[{id:"minecraft:armor_stand",Invisible:1b,Silent:1b,DisabledSlots:2096896,Tags:["ap-rm_portal"]}]}
data modify entity @e[tag=ap-rm_portal,tag=!ap-rm_portal_setup,sort=nearest,limit=1] Rotation[0] set from entity @s Rotation[0]
